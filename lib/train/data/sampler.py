...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 num_search_frames, num_template_frames=1, num_dynamic_frames=0,processing=no_processing, frame_sample_mode='causal',
        ...
        self.num_dynamic_frames = num_dynamic_frames
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    gap = 25
                    dynamic_frames = []
                    while None in dynamic_frames or len(
                            dynamic_frames) == 0 or search_frame_ids is None:
                        dynamic_frames = []
                        template_frame_id1 = self._sample_visible_ids(visible, max_id=len(visible) - self.num_search_frames -
                                                                                      self.num_dynamic_frames - gap)  # the initial template id
                        if template_frame_id1 is None:
                            gap -= 5
                            continue
                        #print(template_frame_id1, "template_frame_id1")
                        search_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_search_frames,
                                                                    min_id=template_frame_id1[0] + 1 + self.num_dynamic_frames + gap,
                                                                    max_id=template_frame_id1[0] + self.num_dynamic_frames +
                                                                           gap + self.max_gap + gap_increase)  # the search region id

                        if search_frame_ids is None:
                            gap_increase += 5
                            continue

                        f_id = self._sample_visible_ids(visible, num_ids=self.num_dynamic_frames,
                                                        min_id=template_frame_id1[0] + 1 + gap,
                                                        max_id=search_frame_ids[0])  # the search region id
                        if f_id is None:
                            dynamic_frames += [None]
                        else:
                            dynamic_frames += f_id

                        template_frame_ids = template_frame_id1
                        gap_increase += 5
                    dynamic_frames.sort()
                    dynamic_frames_ids = dynamic_frames
                ...
                dynamic_frames_ids = [1] * self.num_dynamic_frames
                search_frame_ids = [1] * self.num_search_frames
            try:
                template_frames, template_anno, t_meta_obj_train = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict)
                dynamic_frames, dynamic_anno, d_meta_obj_train = dataset.get_frames(seq_id, dynamic_frames_ids, seq_info_dict)
                ...
                dynamic_masks = dynamic_anno['mask'] if 'mask' in dynamic_anno else [torch.zeros((H, W))] * self.num_dynamic_frames
                search_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros((H, W))] * self.num_search_frames

                data = TensorDict({'template_images': template_frames,
                                   'template_anno': template_anno['bbox'],
                                   'template_masks': template_masks,
                                   'dynamic_images': dynamic_frames,
                                   'dynamic_anno': dynamic_anno['bbox'],
